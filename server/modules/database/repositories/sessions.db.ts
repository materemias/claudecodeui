    ...
    if (existing) {
      db.prepare(
        `UPDATE sessions SET
           ...
           jsonl_path = ?,
           is_one_shot = ?,
           isArchived = CASE WHEN ? THEN isArchived WHEN ? IS NULL OR julianday(?) > julianday(updated_at) THEN 0 ELSE isArchived END,
           custom_name = CASE
    ...
    db.prepare(
      `INSERT INTO sessions (session_id, provider, provider_session_id, custom_name, name_source, project_path, jsonl_path, is_one_shot, isArchived, created_at, updated_at)
       ...
       ON CONFLICT(session_id) DO UPDATE SET
         ...
         isArchived = CASE WHEN ? THEN sessions.isArchived WHEN ? IS NULL OR julianday(excluded.updated_at) > julianday(sessions.updated_at) THEN 0 ELSE sessions.isArchived END,
         custom_name = CASE
