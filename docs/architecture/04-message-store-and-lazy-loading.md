...
| --- | --- |
| `messagesRepresentSamePersistedRow` | "Are these the same disk row?" Same `id` wins outright. Otherwise `provider`, `kind`, `timestamp` and `role` must all match, and then the first discriminator the pair has: `toolId`, else `rowid`, else `sequence`, else the full content tuple (`content`, `text`, `toolName`, `commandName`, `parentToolUseId`, `status`, `summary`, serialized `toolInput`). The fallback exists because Codex mints fresh ids on every read. A pair whose every tuple field is empty on both sides is reported **distinct**: a status note carries its payload in `status`/`summary` alone, and a burst of them shares one timestamp, so comparing nothing used to declare a whole page of them identical and stall the pager. |
